...
class _ConsoleState extends State<Console> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(child: Eyebrow('SQL console · ${state.role}')),
            SpChip(
              'Saved${state.saved.isNotEmpty ? ' · ${state.saved.length}' : ''}',
              icon: 'bookmark',
              padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5),
              onTap: () => _savedSheet(context),
            ),
            ...
            children: e.snippets
                .map(
                  (s) => Padding(
                    ...
                    child: SpChip(s[0], onTap: () => _setSql(s[1])),
                  ),
                )
                .toList(),
          ),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: SpButton(
                ...
                sm: true,
                onTap: () =>
                    _setSql(SqlEngine.formatSql(state.sql, state.engine)),
              ),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: SpButton(
                ...
                icon: 'gauge',
                kind: BtnKind.ghost,
                onTap: () => state.explainCurrent(state.sql),
              ),
            ),
            const SizedBox(width: 10),
            Expanded(
              ...
              child: isBatch
                  ? SpButton(
                      label: 'Run all · $stmtCount',
                      icon: 'layers',
                      kind: BtnKind.primary,
                      onTap: () => state.runScript(state.sql),
                    )
                  : SpButton(
                      label: 'Run',
                      icon: 'play',
                      kind: BtnKind.primary,
                      onTap: () => _attemptRun(context),
                    ),
            ),
          ],
        ),
  ...
  ({String verb, bool noWhere, bool prod})? _guardCheck(BuildContext context) {
    ...
    final m = RegExp(
      r'^\s*(update|delete|drop|truncate)',
      caseSensitive: false,
    ).firstMatch(s);
    if (m == null) return null;
    final verb = m.group(1)!.toLowerCase();
    final noWhere =
        RegExp(r'^(update|delete)', caseSensitive: false).hasMatch(s) &&
        !RegExp(r'\bwhere\b', caseSensitive: false).hasMatch(s);
    if (!state.isProd() && !noWhere && verb != 'drop' && verb != 'truncate')
      return null;
  ...
  void _guardDialog(
    ...
    ({String verb, bool noWhere, bool prod}) g,
  ) {
    final state = context.read<AppState>();
    showSpDialog(context, (ctx) {
      ...
      return SpDialog(
        ...
        iconColor: c.danger,
        title: g.noWhere
            ? '${g.verb.toUpperCase()} without WHERE'
            : 'Confirm ${g.verb.toUpperCase()}',
        sub: Text(
          g.noWhere
  ...
  void _saveDialog(BuildContext context) {
    ...
    showSpDialog(
      ...
      (ctx) => SpDialog(
        icon: 'bookmark',
        title: 'Save query',
        sub: const Text('Stored on this device only.'),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            SpInput(
          ...
          return SpSheet(
            ...
            child: state.saved.isEmpty
                ? const Empty(
                    ...
                    sub:
                        'Write a query and tap the bookmark to save it on this device.',
                  )
                : Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: state.saved
                        .map(
                          (s) => Padding(
...
// Maps raw server errors to a one-line, human-facing summary.
String _friendlyError(String m) {
  final code = RegExp(r'([0-9A-Za-z]{5})\b').firstMatch(m)?.group(1) ?? '';
  if (code == '42P01' || m.contains('does not exist'))
    return 'Unknown table or view — check the name. (SQLSTATE $code)';
  if (code == '42703') return 'Unknown column in this query. (SQLSTATE $code)';
  if (code == '42601' || m.toLowerCase().contains('syntax error'))
    return 'SQL syntax error — review the statement. (SQLSTATE $code)';
  if (code == '23505')
    return 'A row with that key already exists. (SQLSTATE $code)';
  if (code == '42501')
    return 'Permission denied for this action. (SQLSTATE $code)';
  if (code == '23503')
    return 'Referenced row exists elsewhere — fix the relationship first. (SQLSTATE $code)';
  if (code == '42P07') return 'That object already exists. (SQLSTATE $code)';
  if (code == '23514')
    return 'Value violates a CHECK constraint. (SQLSTATE $code)';
  if (code == '23000' || code == '23001')
    return 'Value violates a constraint. (SQLSTATE $code)';
...
class _ResultsPanelState extends State<ResultsPanel> {
  ...
  Widget build(BuildContext context) {
        ...
        child: Column(
          ...
          children: const [
            ...
            Empty(
              ...
              sub: 'Build a query or run a statement to see rows here.',
            ),
          ],
        ),
      );
    ...
    final tabular =
        !res.denied && !res.error && !res.status && res.headers != null;
    final srcName =
        (RegExp(
          r'''from\s+[`"\[]?(\w+)''',
          caseSensitive: false,
        ).firstMatch(state.sql)?.group(1)) ??
        'result';
        ...
        children: [
          Row(
            children: [
              ...
              Text('${res.ms} ms', style: mono(size: 11, color: c.text3)),
            ],
          ),
          if (hasRows) ...[
              ...
              child: Row(
                ...
                children: [
                  ...
                  SpChip(
                    ...
                    onTap: () =>
                        showSpDialog(context, (_) => const PinDialog()),
                  ),
                  const SizedBox(width: 8),
                  SizedBox(
                    width: 138,
          ...
          if (res.denied || res.error)
            SpCard(
              color: res.denied ? c.warningSoft : c.dangerSoft,
              borderColor: Colors.transparent,
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      SpIcon(
                        res.denied ? 'lock' : 'alert',
                        size: 17,
                        color: res.denied ? c.warning : c.danger,
                      ),
                      const SizedBox(width: 9),
                      Text(
                        res.denied ? 'Access denied' : 'Statement error',
                        style: sans(
                          size: 13,
                          weight: FontWeight.w700,
                          color: res.denied ? c.warning : c.danger,
                  ...
                  if (res.error) ...[
                    ...
                    if ((res.message ?? '').isNotEmpty) ...[
                      ...
                      Text(
                        res.message!,
                        style: mono(size: 11.5, color: c.text3, height: 1.5),
                      ),
                    ],
                  ] else
                    Text(
                      res.message ?? '',
                      style: mono(size: 12, color: c.text2, height: 1.5),
                    ),
                ],
              ),
            )
            ...
            SpCard(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      SpIcon('check', size: 17, color: c.success),
                      const SizedBox(width: 9),
                      Text(
                        '${res.statementType} executed',
                        style: sans(
                          size: 13,
                          weight: FontWeight.w700,
                          color: c.success,
                  ...
                  Text(
                    res.comment ?? '',
                    style: mono(size: 12, color: c.text2),
                  ),
                ],
              ),
            )
  ...
  Widget build(BuildContext context) {
    ...
    if (rows.isEmpty)
      return const Empty(
        icon: 'search',
        title: 'No rows',
        sub: 'The query ran successfully but matched no rows.',
      );
    Widget body;
    if (view == 'cards') {
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(comment, style: mono(size: 11, color: c.text3)),
        ),
      ],
    );
  }
  ...
  String _toJson(List<String> headers, List<List<Object?>> rows) {
    ...
    for (var ri = 0; ri < rows.length; ri++) {
      ...
      for (var i = 0; i < headers.length; i++) {
        ...
        final val = v == null
            ? 'null'
            : (v is num ? '$v' : '"${'$v'.replaceAll('"', '\\"')}"');
        buf.write(
          '    "${headers[i]}": $val${i < headers.length - 1 ? ',' : ''}\n',
        );
      }
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          '${headers[0]}: ${row.isNotEmpty && row[0] != null ? row[0] : 'NULL'}',
                          overflow: TextOverflow.ellipsis,
                          style: mono(
                            size: 12.5,
                            weight: FontWeight.w700,
                ...
                Padding(
                  ...
                  child: Column(
                    children: headers.skip(1).toList().asMap().entries.map((e) {
                      final i = e.key;
                      final h = e.value;
...
class _ResultChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < headers.length; i++) {
      if (rows.any((r) => i < r.length && r[i] is num) &&
          rows.every((r) => i >= r.length || r[i] == null || r[i] is num))
        numericCols.add(i);
    }
    if (numericCols.isEmpty)
      return const Empty(
        icon: 'chart',
        title: 'No numeric column',
        sub:
            'Add an aggregate or numeric column to visualize results as a chart.',
      );
    bool isId(String h) =>
        RegExp(r'(^|_)id$|_id$', caseSensitive: false).hasMatch(h);
    final numIdx = numericCols.firstWhere(
      (i) => !isId(headers[i]),
      orElse: () => numericCols.first,
    );
    var labelIdx = -1;
    for (var i = 0; i < headers.length; i++) {
    ...
    final data = rows.take(24).toList().asMap().entries.map((e) {
      ...
      return (
        label: labelIdx == -1
            ? '#${e.key + 1}'
            : (labelIdx < r.length && r[labelIdx] != null
                  ? '${r[labelIdx]}'
                  : 'NULL'),
        val: numIdx < r.length && r[numIdx] is num ? r[numIdx] as num : 0,
      );
    }).toList();
    final max = data
        .map((d) => d.val.abs())
          ...
          Row(
            children: [
              ...
              Expanded(
                child: RichText(
                  text: TextSpan(
                    style: mono(size: 11.5, color: c.text2),
                    children: [
                      TextSpan(text: headers[numIdx]),
                      TextSpan(
                        text: ' by ',
                        style: mono(size: 11.5, color: c.text4),
                      ),
                      TextSpan(
                        text: labelIdx == -1 ? 'row' : headers[labelIdx],
                      ),
                    ],
                  ),
                ),
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(
                    width: 56,
                    child: Text(
                      d.val == d.val.roundToDouble()
                          ? '${d.val.toInt()}'
                          : d.val.toStringAsFixed(2),
                      textAlign: TextAlign.right,
                      style: mono(size: 11, color: c.synNum),
                    ),
                  ),
                ],
              ),
      ...
      onSelected: (fmt) async {
        ...
        if (fmt == 'copy' && context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(r == 'ok' ? 'Copied JSON' : 'Copy failed'),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Eyebrow('Script · ${res.batch!.length} statements'),
              ),
              Text('${res.ms} ms', style: mono(size: 11, color: c.text3)),
            ],
          ),
          ...
          Row(
            children: [
              ...
              if (failed > 0) ...[
                const SizedBox(width: 8),
                SpBadge('$failed failed', variant: 'err'),
              ],
            ],
          ),
          const SizedBox(height: 12),
                        ...
                        child: Row(
                          children: [
                            ...
                            SpIcon(
                              ok2 ? 'check' : 'alert',
                              size: 14,
                              color: ok2 ? c.success : c.danger,
                            ),
                            const SizedBox(width: 6),
                            Transform.rotate(
                              angle: isOpen ? 3.14159 : 0,
                              child: SpIcon('chevD', size: 15, color: c.text3),
                            ),
                          ],
                        ),
                      ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (it.res.headers != null)
                              ResultTable(
                                headers: it.res.headers!,
                                rows: it.res.rows!.take(20).toList(),
                              )
                            else
                              Text(
                                it.res.message ?? it.res.comment ?? '',
                                style: mono(
                                  size: 11.5,
                                  color: ok2 ? c.text2 : c.danger,
                                ),
                              ),
                          ],
                        ),
...
class _ExplainViewState extends State<_ExplainView> {
  ...
  Widget build(BuildContext context) {
    ...
    Color costColor(String cost) =>
        {'HIGH': c.danger, 'MEDIUM': c.warning, 'LOW': c.success}[cost] ??
        c.success;
    Color costSoft(String cost) =>
        {
          'HIGH': c.dangerSoft,
          'MEDIUM': c.warningSoft,
          'LOW': c.successSoft,
        }[cost] ??
        c.successSoft;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Segmented<String>(
                  ...
                  items: const [
                    SegItem('tree', '', icon: 'dotgrid'),
                    SegItem('detail', '', icon: 'rows'),
                  ],
                ),
              ),
            ],
          ...
          const SizedBox(height: 10),
          SpCard(
            color: high ? c.warningSoft : c.successSoft,
            borderColor: Colors.transparent,
            padding: const EdgeInsets.all(14),
            child: Row(
              children: [
                SpIcon(
                  high ? 'alert' : 'check',
                  size: 18,
                  color: high ? c.warning : c.success,
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        high
                            ? 'A full table scan was detected in this plan.'
                            : 'All steps use indexes efficiently.',
                        style: sans(size: 11.5, color: c.text2),
                      ),
                    ],
                  ),
                ),
          ...
          if (view == 'tree')
            ...
            ...nodes.map((n) {
              ...
              return Padding(
                ...
                child: SpCard(
                  borderColor: col.withOpacity(0.33),
                  padding: const EdgeInsets.all(14),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: _field(
                              'ACCESS',
                              n.access,
                              color: n.access == 'ALL' ? c.warning : c.success,
                            ),
                          ),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: _field(
                              'KEY USED',
                              n.key,
                              color: n.key == 'NULL' ? c.text3 : c.accent,
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _advice(SpColors c, String advice, Color col, String cost) =>
      Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                advice,
                style: mono(size: 11.5, color: c.text2, height: 1.55),
              ),
            ),
          ],
        ),
        ...
        SpCard(
          ...
          child: Column(
            children: chain.asMap().entries.map((entry) {
              ...
              final n = entry.value;
              final col = costColor(n.cost);
              final widthPct = ((n.rows / maxRows) * 100)
                  .clamp(14, 100)
                  .round();
              final isSel = sel == i;
              return Column(
                children: [
                  if (i > 0)
                    Container(width: 2, height: 22, color: c.borderStrong),
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: n.result
                            ? c.surface3
                            : (isSel ? costSoft(n.cost) : c.surface2),
                        borderRadius: BorderRadius.circular(12),
                        border: Border.all(
                          color: n.result
                              ? c.border2
                              : (isSel ? col : Colors.transparent),
                          width: 1.5,
                        ),
                      ),
                      child: Column(
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                height: 26,
                                decoration: BoxDecoration(
                                  color: n.result ? c.accent : costSoft(n.cost),
                                ...
                                child: SpIcon(
                                  n.result
                                      ...
                                      : (n.access == 'ALL' ||
                                            ...
                                            : 'key'),
                                  size: 15,
                                  color: n.result ? c.accentInk : col,
                                ),
                              ),
                              const SizedBox(width: 9),
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (n.table.isNotEmpty)
                                      Text(
                                        n.table,
                                        style: mono(size: 10, color: c.text3),
                                      ),
                                  ],
                                ),
                              ),
