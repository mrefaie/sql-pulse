  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        Text(
          '${pins.length} pinned quer${pins.length == 1 ? 'y' : 'ies'} · live on-device',
        ...
        if (pins.isEmpty)
          const Empty(
            icon: 'grid',
            title: 'No pinned cards yet',
            sub:
                'Run a query, then tap "Pin" in the results to add a live metric, chart, or table here.',
          )
        else
          ...pins.map(
            (pin) => Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _PinCard(pin: pin),
            ),
          ),
      ],
    );
...
class _PinCard extends StatelessWidget {
      ...
      builder: (context, snap) {
        ...
        if (!snap.hasData) {
          ...
          body = Text(
            ...
            style: mono(size: 11.5, color: c.danger),
          );
        } else {
          body = pin.viz == 'metric'
              ? _metricViz(c, res!)
  ...
  Widget _metricViz(SpColors c, QueryResult res) {
    if (res.headers == null || res.rows!.isEmpty)
      return Text('No data', style: sans(size: 12, color: c.text3));
    ...
    if (res.rows!.length == 1 && ni >= 0) {
      ...
      val = res.rows!.fold<num>(
        0,
        (a, r) => a + (ni < r.length && r[ni] is num ? r[ni] as num : 0),
      );
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          style: sans(size: 11.5, weight: FontWeight.w600, color: c.text3),
        ),
      ],
    );
  }
  ...
  Widget _chartViz(SpColors c, QueryResult res) {
    ...
    final data = res.rows!.take(8).toList().asMap().entries.map((e) {
      final r = e.value;
      return (
        label: li < 0 ? '#${e.key + 1}' : '${li < r.length ? r[li] : 'NULL'}',
        val: ni < r.length && r[ni] is num ? r[ni] as num : 0,
      );
    }).toList();
    final max = data
        .map((d) => d.val.abs())
                ...
                children: [
                  SizedBox(
                    ...
                    child: Text(
                      ...
                      overflow: TextOverflow.ellipsis,
                      style: mono(size: 10.5, color: c.text2),
                    ),
                  ),
                  const SizedBox(width: 9),
                  Expanded(
                  ...
                  SizedBox(
                    width: 50,
                    child: Text(
                      d.val is int || d.val == d.val.roundToDouble()
                          ? '${d.val.toInt()}'
                          : d.val.toStringAsFixed(1),
                      textAlign: TextAlign.right,
                      style: mono(size: 10.5, color: c.synNum),
                    ),
                  ),
                ],
              ),
  ...
  Widget _tableViz(QueryResult res) {
    if (res.headers == null)
      return Text(res.comment ?? 'No rows', style: const TextStyle());
    return ResultTable(
      headers: res.headers!,
      rows: res.rows!.take(5).toList(),
      showRowNum: false,
      flat: true,
    );
  }
...
class _PinDialogState extends State<PinDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return SpDialog(
      ...
      title: 'Pin to board',
      sub: const Text(
        'Saved on-device and re-run live each time you open the board.',
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: viz == v[0] ? c.accentSoft : c.surface2,
                        borderRadius: BorderRadius.circular(12),
                        border: Border.all(
                          color: viz == v[0] ? c.accent : c.border,
                        ),
                      ),
                      child: Column(
                        children: [
                          SpIcon(
                            v[1],
                            size: 18,
                            color: viz == v[0] ? c.accent : c.text3,
                          ),
                          ...
                          Text(
                            ...
                            style: sans(
                              ...
                              color: viz == v[0] ? c.accent : c.text2,
                            ),
                          ),
                        ],
                      ),
