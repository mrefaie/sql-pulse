...
ColumnDef colFromJson(Map j) => ColumnDef(
  ...
  pk: j['pk'] as bool? ?? false,
  nullable: j['nullable'] as bool? ?? true,
  ai: j['ai'] as bool? ?? false,
  fkTable: j['fkTable'] as String?,
  fkCol: j['fkCol'] as String?,
  def: j['def'] as String? ?? '',
);
...
TableDef tableFromJson(Map j) => TableDef(
  ...
  indexes: (j['indexes'] as List? ?? [])
      .map(
        (i) => IndexDef(
          name: i['name'] as String,
          columns: (i['columns'] as List).cast<String>(),
          unique: i['unique'] as bool? ?? false,
        ),
      )
      .toList(),
);
...
Map<String, dynamic> catalogToJson(Catalog c) => {
  ...
  'triggers': c.triggers,
  'er': {
    for (final e in c.er.entries) e.key: [e.value.x, e.value.y],
  },
  'relations': c.relations
      .map(
        (r) => {'from': r.from, 'to': r.to, 'label': r.label, 'kind': r.kind},
      )
      .toList(),
};
...
Catalog catalogFromJson(Map j) {
  ...
  (j['tables'] as Map).forEach(
    (k, v) => tables[k as String] = tableFromJson(v as Map),
  );
  return Catalog(
    label: j['label'] as String? ?? '',
    tables: tables,
    views: (j['views'] as List)
    ...
    triggers: (j['triggers'] as List)
        ...
        .toList(),
    er: {
      for (final e in (j['er'] as Map).entries)
        e.key as String: Offset2(
...
PlanNode planFromJson(Map j) => PlanNode(
  ...
  possible: j['possible'] as String? ?? '',
  key: j['key'] as String? ?? '',
  rows: (j['rows'] as num?)?.toInt() ?? 0,
  cost: j['cost'] as String? ?? 'LOW',
  advice: j['advice'] as String? ?? '',
  result: j['result'] as bool? ?? false,
);
...
Map<String, dynamic> resultToJson(QueryResult r) => {
  ...
  'batch': r.batch
      ?.map((b) => {'sql': b.sql, 'res': resultToJson(b.res)})
      .toList(),
};
...
QueryResult resultFromJson(Map j) => QueryResult(
  ...
  batch: (j['batch'] as List?)
      ?.map(
        (b) => BatchItem(b['sql'] as String, resultFromJson(b['res'] as Map)),
      )
      .toList(),
);

List<RowMap> rowsFromJson(List j) =>
    j.map((r) => (r as Map).cast<String, Object?>()).toList();
