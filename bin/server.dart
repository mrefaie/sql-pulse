...
DbDriver makeServerDriver(String engine) {
  ...
  switch (engine) {
    ...
    case 'sqlite':
      throw DbException('SQLite is a local-file engine and is not available over the web. Use the desktop app, or a networked engine (PostgreSQL / MySQL / MariaDB / SQL Server).');
    default:
      return TunneledDriver(PostgresDriver());
  }
}
...
Future<void> main(List<String> args) async {
  ...
  Future<Response> handle(Request req, Future<Map<String, dynamic>> Function(Map<String, dynamic> body) fn) async {
    try {
      ...
      return _json({'ok': false, 'error': e is DbException ? e.message : e.toString()});
    }
  }
  ...
  router.post('/api/connect', (Request r) => handle(r, (b) async {
  ...
  router.post('/api/catalogs', (Request r) => handle(r, (b) async {
        return {'catalogs': await session(b).listCatalogs()};
      }));

  router.post('/api/introspect', (Request r) => handle(r, (b) async {
        final cat = await session(b).introspect(b['catalog'] as String);
        return {'catalog': catalogToJson(cat)};
      }));

  router.post('/api/execute', (Request r) => handle(r, (b) async {
        final res = await session(b).execute(b['sql'] as String, catalog: b['catalog'] as String?);
        return {'result': resultToJson(res)};
      }));

  router.post('/api/explain', (Request r) => handle(r, (b) async {
        final res = await session(b).explain(b['sql'] as String, catalog: b['catalog'] as String?);
        return {'result': resultToJson(res)};
      }));

  router.post('/api/txbatch', (Request r) => handle(r, (b) async {
        final stmts = (b['stmts'] as List).cast<String>();
        final res = await session(b).runTransaction(stmts, catalog: b['catalog'] as String?);
        return {'result': res == null ? null : resultToJson(res)};
      }));

  router.post('/api/preview', (Request r) => handle(r, (b) async {
        final rows = await session(b).preview(b['catalog'] as String, b['table'] as String, (b['limit'] as num?)?.toInt() ?? 60);
        return {'rows': rows};
      }));

  router.post('/api/close', (Request r) => handle(r, (b) async {
        final d = _sessions.remove(b['session']);
        await d?.close();
        return {};
      }));

  router.get('/api/health', (Request r) => _json({'ok': true, 'sessions': _sessions.length}));
  ...
  Handler spa(Handler next) => (Request req) async {
        ...
        if (res.statusCode == 404 && req.method == 'GET' && !req.url.path.startsWith('api/')) {
          return createStaticHandler('build/web', defaultDocument: 'index.html')(req.change(path: ''));
        }
        return res;
      };
  ...
  stdout.writeln('SQL Pulse server on http://0.0.0.0:${server.port}  (web: ${webDir.existsSync() ? 'ready' : 'MISSING build/web'})');
}

Response _json(Map<String, dynamic> body) => Response.ok(jsonEncode(body), headers: {'content-type': 'application/json'});
...
Middleware _log() => (Handler inner) => (Request req) async {
      final res = await inner(req);
